...
export class ResultViewComponent {
  propositions$ = this.store
    .select(CinePsSelectors.propositions)
    .pipe(map((propositions) => [...propositions].sort(byScore)));
  havingVotedCount$ = this.store.select(CinePsSelectors.havingVotedCount);
  havingVotedMembers$ = this.store.select(
    CinePsSelectors.slices.havingVotedMembers
  );
